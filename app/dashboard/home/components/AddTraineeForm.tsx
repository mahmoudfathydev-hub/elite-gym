...
const AddTraineeForm = ({ onAddTrainee }: AddTraineeFormProps) => {
    ...
    const uploadImage = async (file: File) => {
        try {
            ...
            try {
                ...
                console.log("Image URL from Appwrite:", imageUrl)
            } catch (urlError) {
                ...
                console.log("Image URL (fallback):", imageUrl)
            }
            ...
            alert(`Image upload failed. Continuing without image. Error: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }   
    ...
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault()
        if (!formData.fullName || !formData.weight || !formData.height) return
        
        try {
            console.log("Submitting trainee data:", formData)
            console.log("Database ID:", appwriteConfig.databaseId)
            console.log("Collection name: trainee")
            
            // Test basic Appwrite connection first
            ...
            console.log("Trainee created successfully:", response)
            ...
            console.error("Error details:", JSON.stringify(error, null, 2))
            alert(`Failed to register trainee. Error: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
